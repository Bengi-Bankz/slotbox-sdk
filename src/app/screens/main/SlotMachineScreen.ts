...
export class SlotMachineScreen extends Container {
    ...
    private calculateScale(): void {
        ...
        console.log(`Scale calculation - Screen: ${screenWidth}x${screenHeight}, Mobile: ${isMobile}, Landscape: ${isLandscape}`);
        console.log(`Min dimension: ${Math.min(screenWidth, screenHeight)}, Mobile threshold: 768`);
        ...
        if (isMobile && !isLandscape) {
            ...
            const availableHeight = screenHeight - (verticalPadding * 2) - buttonSpaceHeight;
            ...
            const symbolScale = Math.min(maxSymbolWidth / baseSymbolSize, maxSymbolHeight / baseSymbolSize);
            ...
            this.reelWidth = (this.scaleFactor * baseSymbolSize) + 2;
            this.symbolSize = baseSymbolSize * this.scaleFactor;

            console.log(`Mobile Portrait - Symbol Scale: ${symbolScale}, Final: ${this.scaleFactor}, Reel Width: ${this.reelWidth}`);
        }
            ...
            const availableWidth = screenWidth - (horizontalPadding * 2) - buttonSpaceWidth;
            ...
            console.log(`Mobile Landscape - Width Scale: ${widthScale}, Height Scale: ${heightScale}, Final: ${this.scaleFactor}`);
        }
        else {
            ...
            console.log(`Desktop - Width Scale: ${widthScale}, Height Scale: ${heightScale}, Final: ${this.scaleFactor}`);
        }

        console.log(`Final dimensions - Reel: ${this.reelWidth}, Symbol: ${this.symbolSize}`);
    }

    private initSlotTextures(): void {
        // Create colored rectangle textures as placeholders
        // In a real game, these would be your actual symbol textures
        const colors = [0xff0000, 0x00ff00, 0x0000ff, 0xffff00, 0xff00ff, 0x00ffff, 0xffa500, 0x800080];

        colors.forEach((color, index) => {
            const graphics = new Graphics();
            graphics.rect(0, 0, this.symbolSize, this.symbolSize);
            graphics.fill({ color });
    ...
    private createSlotMachine(): void {
        ...
        if (isMobile && !isLandscape) {
            ...
            this.reelContainer.y = Math.round((availableHeight - totalReelHeight) / 2);
    ...
    private createTitle(): void {
        ...
        if (isMobile && !isLandscape) {
            ...
            const titleStyle = new TextStyle({
                ...
                fontWeight: 'bold',
                fill: { fill },
                stroke: { color: 0x4a1850, width: Math.max(2, Math.floor(3 * this.scaleFactor)) },
                dropShadow: {
                    color: 0x000000,
                    angle: Math.PI / 6,
                    blur: 4,
            ...
            const titleSpaceTop = Math.max(10, this.reelContainer.y - titleText.height - 10);
    ...
    private createUI(): void {
        ...
        this.spinButton = new FancyButton({
            defaultView: new Graphics().rect(0, 0, scaledButtonWidth, scaledButtonHeight).fill({ color: 0x00aa00 }),
            text: new Text('SPIN!', {
                fontSize: Math.floor(24 * this.scaleFactor),
                fill: 0xffffff,
                fontWeight: 'bold'
        ...
        const reelBottom = this.reelContainer.y + (this.symbolSize * this.SYMBOLS_PER_REEL);
        const reelRight = this.reelContainer.x + (this.reelWidth * this.REELS_COUNT);
        const reelCenterY = this.reelContainer.y + (this.symbolSize * this.SYMBOLS_PER_REEL) / 2;

        // Debug logging
        console.log(`Button positioning - Mobile: ${isMobile}, Landscape: ${isLandscape}, Screen: ${screenWidth}x${screenHeight}`);
        console.log(`Reel bottom: ${reelBottom}, Reel right: ${reelRight}, Reel center Y: ${reelCenterY}`);
        ...
        if (isMobilePortrait) {
            ...
            console.log(`Button positioned at: ${this.spinButton.x}, ${this.spinButton.y}`);
        } else {
    ...
    private addViewportBoundary(): void {
        ...
        const infoText = new Text(`Viewport: ${engine().screen.width}x${engine().screen.height}`, {
            fontSize: 14,
            fill: 0xffffff,
            fontWeight: 'bold'
        });
