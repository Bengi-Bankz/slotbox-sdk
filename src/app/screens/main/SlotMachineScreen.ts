...
interface Reel {
  ...
  blur: BlurFilter;
  [key: string]: number | Container | Sprite[] | BlurFilter; // Index signature for tweening
}
...
export class SlotMachineScreen extends Container {
  ...
  private createSlotMachine(): void {
    ...
    // Determine device type and orientation for sizing
    const screenWidth = engine().screen.width;
    const screenHeight = engine().screen.height;
    const isLandscape = screenWidth > screenHeight;
    const isMobile = Math.min(screenWidth, screenHeight) < 768;
    ...
    const innerPadding = 10; // Extra padding around the entire grid

    // Base dimensions for the blue box
    const baseBlueWidth =
      this.REELS_COUNT * this.symbolSize +
      (this.REELS_COUNT - 1) * reelSpacing +
      2 * innerPadding;
    const baseBlueHeight =
      this.SYMBOLS_PER_REEL * this.symbolSize +
      (this.SYMBOLS_PER_REEL - 1) * symbolSpacing +
      2 * innerPadding;

    // Calculate additional padding for the red outer box based on screen type
    let outerPadding = 15; // Default for mobile portrait
    if (!isMobile || isLandscape) {
      // Bigger padding for desktop and landscape views
      outerPadding = isMobile ? 25 : 35; // 25px for mobile landscape, 35px for desktop
    }

    // Red outer box dimensions
    const redBoxWidth = baseBlueWidth + 2 * outerPadding;
    const redBoxHeight = baseBlueHeight + 2 * outerPadding;

    // Create the red outer background box
    const redBackground = new Graphics();
    redBackground.roundRect(0, 0, redBoxWidth, redBoxHeight, 15); // Slightly larger corner radius
    redBackground.fill({ color: 0xff4444 }); // Red background
    redBackground.stroke({ color: 0x000000, width: 4 }); // Black border around the red box
    this.reelContainer.addChild(redBackground);

    // Create the blue inner background rectangle (positioned inside the red box)
    const blueBackground = new Graphics();
    blueBackground.roundRect(
      outerPadding,
      outerPadding,
      baseBlueWidth,
      baseBlueHeight,
    ...
    blueBackground.fill({ color: 0x87ceeb }); // Baby blue background
    blueBackground.stroke({ color: 0x000000, width: 3 }); // Black border around the blue background
    this.reelContainer.addChild(blueBackground);
    ...
    for (let i = 0; i < this.REELS_COUNT; i++) {
      ...
      // Position reel containers with both outer and inner padding offset and spacing
      rc.x = outerPadding + innerPadding + i * (this.symbolSize + reelSpacing);
      ...
      for (let j = 0; j < this.SYMBOLS_PER_REEL; j++) {
        ...
        // Position symbols with both outer and inner padding and spacing
        symbol.y =
          outerPadding + innerPadding + j * (this.symbolSize + symbolSpacing);
    ...
    const totalReelWidth = redBoxWidth; // Use the red box total width
    const totalReelHeight = redBoxHeight; // Use the red box total height
  ...
  private tweenTo(
    ...
    const tween: Tween = {
      object: object as { [key: string]: number },
      property,
